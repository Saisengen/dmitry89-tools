...
$all = '';
$all = $_GET["all"];
$title = ($all != '') ? 'Кандидаты на удаление по дате номинаций' : 'Просроченные номинации к удалению';
...
$query = ($all != '') ? 'select p.page_namespace, p.page_title, c.cl_sortkey_prefix from ruwiki_p.categorylinks as c, ruwiki_p.page as p where c.cl_to = "Википедия:Кандидаты_на_удаление_по_дате_номинации" and c.cl_from = p.page_id and UNIX_TIMESTAMP(c.cl_timestamp) > UNIX_TIMESTAMP("'.date("Y-m-d", $purge).' 00:00:00") order by c.cl_sortkey_prefix' : 'select p.page_namespace, p.page_title, c.cl_sortkey_prefix from ruwiki_p.categorylinks as c, ruwiki_p.page as p where c.cl_to = "Википедия:Просроченные_подведения_итогов_по_удалению_страниц" and c.cl_from = p.page_id and UNIX_TIMESTAMP(c.cl_timestamp) > UNIX_TIMESTAMP("'.date("Y-m-d", $purge).' 00:00:00") order by c.cl_sortkey_prefix';
