<?php
$title = 'Кандидаты на удаление по дате номинаций';
require_once("../skeleton_1.php");
$ts_pw = posix_getpwuid(posix_getuid());
$ts_mycnf = parse_ini_file($ts_pw['dir'] . "/replica.my.cnf");
$stime = time();
$db = mysql_connect('ruwiki.web.db.svc.wikimedia.cloud', $ts_mycnf['user'], $ts_mycnf['password']);
unset($ts_mycnf, $ts_pw);
mysql_select_db('ruwiki_p', $db) or die(mysql_error()); ;
$th = 0;
$purge = (time() - 86400*365);
$query = 'select p.page_namespace, p.page_title, c.cl_sortkey_prefix from ruwiki_p.categorylinks as c, ruwiki_p.page as p where c.cl_to = "Википедия:Кандидаты_на_удаление_по_дате_номинации" and c.cl_from = p.page_id and UNIX_TIMESTAMP(c.cl_timestamp) > UNIX_TIMESTAMP("'.date("Y-m-d", $purge).' 00:00:00") order by c.cl_sortkey_prefix';
...
$content = '<p>На данной странице представлены <a href="//ru.wikipedia.org/wiki/Категория:Википедия:Кандидаты_на_удаление_по_дате_номинации">все кандидаты на удаление</a>, сортированные по дате номинации. Данные актуальны на момент загрузки страницы с поправкой на задержки в синхронизации базы данных.
...
<br /><br /><b>Всего: '.$number.' номинаций за '.$n.$plural.'. (выполнено за '.$timed.' сек.)</b></p>'.$content;
/*echo "<pre>"; 
print_r($kunom); 
echo "</pre>";*/
mysql_close($db);
